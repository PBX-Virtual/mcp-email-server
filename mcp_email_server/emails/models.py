...
class EmailMetadata(BaseModel):
    ...
    tags: list[EmailTag] = Field(default_factory=list, max_length=APPLICATION_LIMITS.flags)
    tag_names: list[EmailTag] = Field(default_factory=list, max_length=APPLICATION_LIMITS.flags)
    ...
    def from_email(cls, email: dict[str, Any]):
        return cls(
            ...
            tags=[flag for flag in email.get("_flags", []) if isinstance(flag, str) and not flag.startswith("\\")],
        )
